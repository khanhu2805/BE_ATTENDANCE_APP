...
bgimage = cv2.imread('background.jpg')
...
def detect():
    ...
    print(student_code[:2])
    print(student_code[4:6])
    print(student_code[6:])
    model = YOLO(f"{student_code[:2]}/{student_code[4:6]}/{int(student_code[6:])//50*50}-{int(student_code[6:])//50*50 + 49}.pt")
    # model = YOLO(f"{student_code}.pt")
    ...
    bg_image_resized = cv2.resize(bgimage, (w, h))
    ...
    #     return result.names[int(result.boxes.cls[0])]
    # else:
    #     return 'Khong nhan dien duoc'
